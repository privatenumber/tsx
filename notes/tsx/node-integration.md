...
When Node evaluates namespaced CommonJS source itself instead of re-entering `Module._load`, it supplies a reduced `require` without `cache` or `extensions` ([Node contract](../node/cjs-esm-interop.md#load-hook-cjs-evaluation)). For namespaced imports, the file-response CommonJS transform branches add a `createRequire()` banner in both the async and sync hooks ([async transform](../../src/esm/hook/load.ts#L376-L387), [sync transform](../../src/esm/hook/load.ts#L529-L546)). The synchronous composed `commonjs-typescript` fallback adds it too, while the asynchronous fallback does not ([sync fallback](../../src/esm/hook/load.ts#L578-L589), [async fallback](../../src/esm/hook/load.ts#L411-L424)). The banner's parent is an OS-native path with an encoded `?namespace=` suffix rather than a `file:` URL, because a `file:` URL's query is dropped from `parent.filename`, which tsx's patched CJS resolver reads ([Node contract](../node/cjs-loader.md#createrequire-parent-identity)). `tsImport()` registers the CommonJS loader and the ESM loader for the same namespace ([ts-import.ts](../../src/esm/api/ts-import.ts#L28-L62)), so the banner's `createRequire` resolves children through the namespaced CJS loader. Standalone `register({ namespace })` does not register that paired loader, so the child-resolution guarantee is scoped to `tsImport()`.
...
| Reduced bridge `require` | [`src/esm/hook/load.ts`](../../src/esm/hook/load.ts) | [Load-hook CJS evaluation](../node/cjs-esm-interop.md#load-hook-cjs-evaluation) |
| CJS bridge namespace encoding | [`preserve-query.ts`](../../src/cjs/api/module-resolve-filename/preserve-query.ts), [`resolve.ts`](../../src/esm/hook/resolve.ts) | [Load-hook CJS evaluation](../node/cjs-esm-interop.md#load-hook-cjs-evaluation) |
